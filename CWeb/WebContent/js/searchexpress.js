 $(function(){
	 $.cookie('mapID',null,{path:'/'});//删除cookie
...
//获取运单信息
        $("#btn").click(function(){
            ...
            var $ID=$("#Express_id").val();//快递单号
            $.cookie('mapID',$ID,{path:'/',expires:1});
          
         
            $('#iframe').attr('src', $('#iframe').attr('src'));//地图
            var name;
            var tel;
            $.ajax({
                type:"get",
                url:"http://localhost:8080/TestCxfHibernate/REST/Domain/getExpressSheet/"+$ID,//快递单号
                dataType:"json",
            ...
            $.ajax({
                type:"get",
                url:"http://localhost:8080/TestCxfHibernate/REST/Domain/getPath/"+$ID,//快递单号路径
                dataType:"json",
                success:function(data){
                    alert("获取成功");
                    var con = "";
